...
uses
 ...
 function RobotBestPathP(tg_x, tg_y:double; var traj: TTrajectory; var avoid: Tavoid): boolean;
...
const
  //AStarSlack = 0.5;   //total space where the robot can actually move around
  AStarSlack = 0.1;
...
var i,x2,y2: integer;
    dx,vf,tet,dy,dy1,vallorfuzzy,valorfuzzy1,valorfuzzyextra,relipse,j,er,wx,wy: double;
    celrod:Tworldcoord;
...
// vai por como obstaculo as bordas da elipse
      ...
      for x2 := ptl.x to pbr.x do begin
        cur_pnt.x := x2;
        for y2 := ptl.y to pbr.y do begin
          cur_pnt.y := y2;
          AStarGridToWorld(cur_pnt, wx, wy);
          if Dist(wx - x, wy - y) < r then begin
...
// A* modificado
          if dinamica then begin
            dif:=abs(iMap.Grid[CurPnt.x, CurPnt.y].ParentDir-ith);
            G := iMap.Grid[CurPnt.x, CurPnt.y].G +EightWayNeighbours[ith].dist+Eightwaydinamic[dif];
          end else begin
...
//A* modificado
        if dinamica then begin
           dif:=abs(iMap.Grid[CurPnt.x, CurPnt.y].ParentDir-ith);
           newG := iMap.Grid[CurPnt.x, CurPnt.y].G +EightWayNeighbours[ith].dist+Eightwaydinamic[dif];
        end else begin
...
begin
  AStarClear(AStarMap);
// A* modificado
 if direcao then AStarDrawCircleTarget(tg_x, tg_y, cd ,2,AStarMap.GridState);
...
// A* modificado
    if  choque then begin
      if (meetingpoint (st_x,st_y,SpeedMax/2,obs[i].x,obs[i].y,obs[i].vx,obs[i].vy,xmet,ymet)) then begin
        ...
        if rasto then AddAStarObstacleTrail(xmet, ymet, obs[i].r,obs[i].vx,obs[i].vy, AStarMap.GridState);
      end
    end
    else begin
